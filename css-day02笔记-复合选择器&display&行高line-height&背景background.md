...
- 注意：
  ...
  - 同理还有这些标签h1,h2,h3,h4,h5,h6,dt，他们都是**文字类**块级标签，里面不能放其他块级元素。
...
```
常见的行内元素有<a>、<span> 、<strong>、<b>、<em>、<i>、<ins>、<u> 、<del>、<s>等，其中<span>标签最典型的行内元素。有的地方也成内联元素
```
...
| ---------- | ---------------------- | ---------------------- | ---------------- | -------------------------- |
| 块级元素   | 一行只能放一个块级元素 | 可以设置宽度高度       | 容器的100%       | **容器级**可以包含任何标签 |
